...
const TIMESTAMP_PRESETS = ['Just now', '2h', '5h', '1d', '3d', '1w', '3w', '2w']

export function ConversationListEditor({
  conversations,
  add,
  update,
  remove,
        ...
        >
          ...
          >
            <Field label="Username">
              <Input
                value={conv.username}
                onChange={e => update(conv.id, { username: e.target.value })}
              />
            </Field>
            ...
            <Row label="Active now (green dot)">
              <Toggle
                checked={conv.activeNow}
                onChange={v => update(conv.id, { activeNow: v })}
              />
            </Row>

            <Row label="Verified (blue checkmark)">
              <Toggle
                checked={conv.verified}
                onChange={v => update(conv.id, { verified: v })}
              />
            </Row>
